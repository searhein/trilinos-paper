...
Additional utilities for nonlinear analysis include abstraction layers and interfaces for application callbacks, as well as automatic differentiation tools that can provide the derivatives critical to analysis algorithms.
...
inverse problems, image processing, and mesh optimization. Application areas include geophysics, climate science \cite{Perego2022}, structural
...
also includes specialized algorithms such as progressive hedging~\cite{rockafellar1991scenarios} and the primal-dual risk minimization algorithm~\cite{kouri2022primal}.
\item
...
values and gradients for the smooth objective function term using trust regions~\cite{baraldi2023proximal}.
ROL also include an implementation of the trust bundle method~\cite{schramm1992bundle}
...
To solve PDE-constrained optimization problems, users only need to implement the
...
Sacado, along with its Kokkos integration, provides high-performance derivative capabilities to numerous Office of Science and NNSA (National Nuclear Security Administration) extreme scale applications, including: Albany for solid mechanics and land ice modeling~\cite{Salinger2016,MPASAlbany2018};
...
The embedded ensemble propagation consists of propagating a subset of samples gathered into a so-called ensemble through the forward simulation at once.
...
the values of input, output, and state variables for every sample of an ensemble. This type can then be used within the Tpetra solver stack as a template argument for the scalar type.
