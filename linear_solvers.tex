...
%
Such one-level
preconditioners can be used as smoothers within multigrid
...
Local kernels are either implemented in Ifpack2 itslef,
...
\todo{Christian Glusa, should the Teko description be a bit more specific? At the moment, it falls pretty short and lacks details compared to many other packages.}
...
The Stratimikos package provides a unified interface to linear solvers and preconditioners in Trilinos (e.g., from Amesos2, Belos, FROSch, Ifpack2, MueLu).
...
Solver and preconditioner parameters are specified via a Teuchos::ParameterList,
