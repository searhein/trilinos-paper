...
In addition to their stand-alone use, the aforemention node-level solvers may be used as the local solvers for domain decomposition preconditioners (Ifpack2 or FROSch) or as the coarse solvers for multilevel preconditioners (MueLu or FROSch).
...
\todo{Many find it difficult to use Teko. Any helpful references for examples, tutorials, etc.?}
...
\todo{Aren't the right-hand side and solution vectors part of the linear system?}
