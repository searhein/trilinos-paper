...
Compadre achieves performance portability by using hierarchical parallelism within the Kokkos programming model and leveraging the batched QR with pivoting algorithm implemented in Kokkos-Kernels. GMLS stencil generation involves an independent problem to be solved in parallel at the team level with thread and vector level loops within each problem.